    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-6">
            {products.map(product => (
                <Link key={product.id} href={"/products/" + product.id} className="block bg-white shadow-lg rounded-lg overflow-hidden transition-transform transform hover:scale-105 hover:shadow-xl">
                    <Image src={'/' + product.imageUrl} alt="Product image" width={150} height={150} className="w-full h-80 object-cover" />
                    <div className="p-4">
                        ...
                        <p className="text-gray-600 mt-2">${product.price}</p>
                        {
                            ...
                            ?(
                                <button className='bg-red-500 text-white px-4 py-2 rounded-md mt-2' onClick={(e)=>{e.preventDefault();
                                    removeFromCart(product.id)}}>Remove from cart</button>

                            ):(
                                <button className='bg-blue-500 text-white px-4 py-2 rounded-md mt-2' onClick={(e)=>{e.preventDefault();
